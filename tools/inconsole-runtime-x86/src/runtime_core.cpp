...
UiRuntimeSnapshot::UiRuntimeSnapshot() : perf(), battery(), battery_percent(100), scene_id(0), now_ms(0) {}
...
BatteryMonitor::BatteryMonitor() : next_update_ms_(0) {}
...
void BatteryMonitor::discover(Logger *logger) {
    ...
    while ((ent = readdir(dir)) != nullptr) {
        ...
        if (type == "Battery") {
            battery_path_ = base;
            info_.node = ent->d_name;
            break;
        }
    }

    closedir(dir);
    if (logger && !battery_path_.empty()) logger->info("Battery node: " + info_.node);
}
...
const BatteryInfo &BatteryMonitor::update(Logger *logger) {
    ...
    if (info_.has_capacity) info_.capacity = std::max(0, std::min(100, capacity));
    ...
    if (info_.has_voltage) info_.voltage_mv = uv / 1000;
